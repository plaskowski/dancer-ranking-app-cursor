...
class EventImportService {
  ...
  Future<EventImportSummary> _getImportSummary(
    ...
    final existingDancers = await _validator.getExistingDancersByNamesWithVariants(allDancerNamesInImport);
  ...
  Future<EventImportSummary> _performImport(
    ...
    final existingDancers = await _validator.getExistingDancersByNamesWithVariants(allDancerNames);
            ...
            for (final attendance in event.attendances) {
              try {
                ...
                if (attendance.status == 'present') {
                  ...
                  // Save impression if provided for present dancers
                  if (attendance.impression != null && attendance.impression!.isNotEmpty) {
                    await _attendanceService.updateDanceImpression(
                      eventId: eventId,
                      dancerId: dancer.id,
                  ...
                  // Save impression if provided for left dancers
                  if (attendance.impression != null && attendance.impression!.isNotEmpty) {
                    await _attendanceService.updateDanceImpression(
                      eventId: eventId,
                      dancerId: dancer.id,
