...
class _DancerActionsDialogState extends State<DancerActionsDialog> {
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            // Score actions (only for present mode and attendants) - second most common
            if (!widget.isPlanningMode && widget.dancer.isPresent)
              ListTile(
                ...
                title: Text(
                    widget.dancer.hasScore ? 'Edit Score' : 'Assign Score'),
                onTap: () {
            ...
            if (!isPastEvent && !widget.isSummaryMode) ...[
              ...
              ListTile(
                ...
                title: Text(widget.dancer.hasRanking
                    ? 'Change Ranking'
                    : 'Set Ranking'),
                onTap: () {
                  ActionLogger.logUserAction(
                      'DancerActionsDialog', 'set_ranking_tapped', {
                    'dancerId': widget.dancer.id,
                    'eventId': widget.eventId,
              ...
              if (widget.dancer.hasRanking)
                ListTile(
                  ...
                  onTap: () {
                    ActionLogger.logUserAction(
                        'DancerActionsDialog', 'edit_ranking_reason_tapped', {
                      'dancerId': widget.dancer.id,
                      'eventId': widget.eventId,
                      'currentRank': widget.dancer.rankName,
            ...
            if (!widget.isPlanningMode && widget.dancer.isPresent)
              ListTile(
                leading: Icon(Icons.music_note,
                    color: context.danceTheme.danceAccent),
                title: Text(widget.dancer.hasDanced
                    ? 'Edit impression'
                    : 'Record Dance'),
                onTap: () {
                  ActionLogger.logUserAction(
            ...
            if (widget.isPlanningMode &&
                widget.dancer.hasRanking &&
                !isPastEvent)
              ListTile(
                leading: Icon(
  ...
  Future<void> _removeFromEvent(BuildContext context) async {
    ActionLogger.logUserAction(
        'DancerActionsDialog', 'remove_from_event_started', {
      'dancerId': widget.dancer.id,
      'eventId': widget.eventId,
      'dancerName': widget.dancer.name,
    });
    ...
    try {
      ...
      ActionLogger.logUserAction(
          'DancerActionsDialog', 'remove_from_event_success', {
        'dancerId': widget.dancer.id,
        'eventId': widget.eventId,
        'dancerName': widget.dancer.name,
      });
      ...
      if (context.mounted) {
        Navigator.pop(context);
        ToastHelper.showSuccess(
            context, 'Removed ${widget.dancer.name} from event');
      }
    } catch (e) {
      ActionLogger.logError(
          'DancerActionsDialog._removeFromEvent', e.toString(), {
        'dancerId': widget.dancer.id,
        'eventId': widget.eventId,
        'dancerName': widget.dancer.name,
      });
